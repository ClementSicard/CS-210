...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(c => c).view.mapValues(s => s.size).toList.sorted
   ...
   *  It also include the empty subset `List()`.ordOccurrence
   *
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences.foldRight(List[Occurrences](Nil))) {
      case ((c,t), acc) => acc ++ (for (co <- acc; n <- 1 to t) yield (c, n) :: co)
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y.foldLeft(SortedMap[Char,Int]() ++ x)) { 
      case (m, (c, t)) => if m(c) - t != 0 then m.updated(c, m(c) - t) else m - c
    }.toList
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def helper(occ: Occurrences): List[Sentence] =
      if (occ.isEmpty) List(Nil) else for {
        ...
        s <- helper(subtract(occ, wordOccurrences(w)))
        if !c.isEmpty
      } yield w :: s
