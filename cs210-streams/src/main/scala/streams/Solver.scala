...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = for (i <- b.legalNeighbors.to(LazyList)) yield (i._1, i._2 :: history)
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] =
            if initial.isEmpty then LazyList.empty else {
              ...
              val n = newNeighborsOnly(neighbors, explored)
              val e = explored ++ n.map(_._1)
              initial.head #:: from(initial.tail ++ n, e)
            }
  /**
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(e => done(e._1))
   ...
   * position.
   */
  lazy val solution: List[Move] = if pathsToGoal.isEmpty then Nil else pathsToGoal.map(e => e._2).head.reverse
}
