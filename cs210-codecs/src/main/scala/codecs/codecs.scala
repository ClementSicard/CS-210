...
trait GivenDecoders {
  ...
  given Decoder[Int] = Decoder.fromPartialFunction { case Json.Num(n) if n.isValidInt  =>  n.toInt}
    // Decoder.fromFunction { case }
...
trait PersonCodecs {
  ...
  given Decoder[Person] = 
    ...
    .transform[Person]((n, a) => Person(n, a))
  }
...
trait ContactsCodecs {
  ...
  given Encoder[Contacts] = ObjectEncoder.field[List[Person]]("people").transform[Contacts](user => user.people)
